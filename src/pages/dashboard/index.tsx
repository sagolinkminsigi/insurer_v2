...
export default function Dashboard() {
  ...
  // if (state && state === 'from_signin') {
  //   window.history.go(-(window.history.length - 1))
  //   navigate('/', { replace: true, state: 'from_signin' })
  // }
  ...
  const touchStartY = useRef(0) // 터치 시작 시 Y 좌표
  ...
  const handleTouchMove = (e: TouchEvent) => {
    ...
    // 일정 거리 이상 스크롤된 경우에만 키보드를 닫음 (예: 50px 이상)
    if (deltaY > 50) {
      ...
      if (
        ...
        activeElement.blur() // 포커스 해제 -> 키보드 닫힘
      }
    }
  }

  useEffect(() => {
    // 터치 시작 및 터치 이동 이벤트 리스너 추가
      ...
      <Layout.Body>
        <div className='mb-2 flex items-center justify-between space-y-2'>
          ...
          <div className='flex items-center space-x-2'>
            <Button onClick={async () => signOut && (await signOut())}>
              SignOut
            </Button>
          </div>
          {user?.email}
